...
python3 ../scripts/momentum.py AAPL MSFT GOOGL AMZN TSLA META NVDA SPY QQQ --lookback 20 --topn 5 |> $LOGDIR/momentum_signals.csv

cat $LOGDIR/momentum_signals.csv

echo ""
echo "── Generating orders ────────────────────────────────────────"
python3 ../scripts/gen_momentum_orders.py

echo ""
echo "── Routing orders through risk gate ────────────────────────"
echo "AAPL BUY 100 185.50" ?> python3 ../scripts/risk_check.py && echo "Risk gate: PASS" || echo "Risk gate: REJECTED"
python3 ../scripts/gen_momentum_orders.py
