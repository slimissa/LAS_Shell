/*
 * QShell Pipeline Stage 3 — risk_filter.c  (C implementation)
 * =============================================================
 ...
 *
 * Participates in QShell pipelines with zero changes to pipes.c.
 */
...
static void log_rejection(const Candidate* c) {
    ...
    if (!home) return;
    char path[512];
    snprintf(path, sizeof(path), "%s/.qshell_risk_rejections", home);
    FILE* f = fopen(path, "a");
    if (!f) return;
