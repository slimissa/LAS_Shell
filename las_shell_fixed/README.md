# Las_shell — Quantitative Trading Shell v0.5.5

> A C-native Unix shell where a trading strategy **is** a pipeline —
> paper-trading and backtesting workflows are functional; live-broker
> execution is opt-in and requires a `make CURL=1` rebuild plus explicit
> confirmation (see Configuration and Known Limitations below).
...
│   ├── universe.py             # These 5 top-level .py files are what
│   ├── momentum_filter.py      #   run_pipeline.sh, docs/PIPELINE_CONVENTION.md's
│   ├── risk_filter.py          #   examples, and tests/integration/test_pipeline.sh
│   ├── size_positions.py       #   actually invoke — treat these as canonical.
│   ├── execute.py              #
...
│   │   ├── execute.c
│   │   └── makefile
│   └── python/                 # Byte-identical copies of the 5 files above,
│       ├── universe.py         #   kept only because `make install` still
│       ├── momentum_filter.py  #   references this path. Edit pipeline/*.py
│       ├── risk_filter.py      #   (top level), not these — see the note at
│       ├── size_positions.py   #   the top of each file in this directory.
│       └── execute.py          #
│
...
Honest disclosures, not exhaustive — see `docs/PIPELINE_CONVENTION.md` and
inline `FIX`-tagged comments for more detail on individual items:

- **`execute.py` still can't place a real order.** As of this pass, the
  three pipeline copies (`pipeline/*.py`, `pipeline/python/*.py`,
  `pipeline/src/*.c`) are consistent — `pipeline/python/` is now
  byte-identical to the tested top-level `.py` files, so there's no longer
  a hidden gap between them. But `pipeline/execute.py` was never a real
  broker integration in any copy: it honestly refuses `--mode live` (exit
  1, no fake fill) rather than pretend to trade live. **For live execution,
  use the `order` shell built-in** (`src/broker.c`), not
  `pipeline/execute.py`.
- **C and Python stages are not numerically interchangeable.** Both use
  synthetic (non-real-market) data for testing, and both seed a per-symbol
  RNG the same way — but C's `rand()` and Python's `random` module are
  different algorithms, so a C run and a Python run of the "same" stage
  produce different numbers even from an identical seed. Don't expect
  bit-for-bit parity between `pipeline/src/*.c` and `pipeline/*.py`.
- **Timestamps aren't uniformly UTC.** `audit.c` and `crash_recovery.c` emit
  UTC (`...Z` suffix). The pipeline stages' `meta.timestamp` field
  (`universe.c`, `universe.py`, etc.) use local wall-clock time with no
  offset marker instead. Don't assume `meta.timestamp` is comparable across
  a mixed C/Python/audit-log deployment without accounting for local
  timezone.
- **The roadmap example below needs setup, not just a copy-paste.** The bare
  command names (`universe`, `momentum_filter`, `risk_filter`,
  `size_positions`, `execute`) resolve to `pipeline/bin/*` (built by
  `pipeline/src/makefile`) only if that directory is on your `$PATH` — the
  shell does not add it automatically. `risk_check.py` likewise needs
  `scripts/` on `$PATH` or an explicit path. `flatten` and `pnl` **do**
  resolve automatically (they're registered in `trading_aliases_init.c`,
  loaded by `setmarket`). Live trading additionally requires rebuilding
  with `make CURL=1` (paper mode is the default build).

---

## The End State (from the roadmap)

Illustrative target syntax — requires the `$PATH` setup described above to
run as literally written; `flatten`/`pnl`/`watch`/`assert`/`setmarket`/
`setbroker`/`setaccount`/`setcapital`/`@HH:MM:SS`/`?>`/`|>`/`~>` all work
out of the box in v0.5.5.
