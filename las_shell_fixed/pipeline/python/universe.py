...
canonical pipeline/universe.py (top level), which is what run_pipeline.sh and
...
treat pipeline/universe.py as the source of truth and edit that one.
"""
"""
...
⚠ SYNTHETIC DATA — TESTING ONLY: prices come from a static per-symbol
anchor (SEED_PRICES) perturbed by a single day's random return, reseeded
by symbol+date -- same caveat as scripts/price_feed.py. Useful for
