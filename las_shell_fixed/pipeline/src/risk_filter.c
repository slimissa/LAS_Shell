...
/* FIX CR4: same lookup as extract_num, but also reports whether the key
 * was actually found, so callers can tell "field is 0.0" apart from
 * "field is missing" and fail closed on the latter instead of silently
 * treating a missing signal/price as if it were a real zero. */
static double extract_num_present(const char* json, const char* key, int* present) {
    char search[64];
    snprintf(search, sizeof(search), "\"%s\"", key);
    const char* p = strstr(json, search);
...
static int check(Candidate* c) {
    ...
    /* FIX CR5: there used to be a size/notional check here, gated by
     * 'if (c->size > 0)'. That guard was never true: this stage runs
     * BEFORE size_positions in every documented pipeline order (universe
     * -> momentum_filter -> risk_filter -> size_positions -> execute),
     * so every candidate reaching this function has size == 0 by
     * construction -- it looked like an active risk control but was
     * structurally dead code. The real size/notional enforcement
     * (MAX_POSITION_SIZE / MAX_ORDER_NOTIONAL) now lives in
     * size_positions.c, immediately after size is actually computed. */
    snprintf(c->reason, REASON_LEN, "ok");
    return 1;
}

/* FIX CR2: forward unknown meta fields from the input instead of
 * discarding them -- same pass-through scanner as momentum_filter.c's
 * CM3 fix (duplicated here since each pipeline stage is a standalone
 * translation unit with no shared library). */
static void print_passthrough_meta_fields(const char *obj_json, const char **reserved, int n_reserved) {
    const char *meta_p = strstr(obj_json, "\"meta\"");
    if (!meta_p) return;
    const char *mb = strchr(meta_p, '{');
    if (!mb) return;
    int mdepth = 0; const char *me = mb;
    while (*me) {
        if (*me == '{') mdepth++;
        if (*me == '}') { mdepth--; if (mdepth == 0) { me++; break; } }
        me++;
    }
    const char *p = mb;
    while (p < me && (p = strchr(p, '"')) != NULL) {
        p++;
        const char *key_start = p;
        const char *key_end = strchr(p, '"');
        if (!key_end || key_end >= me) break;
        char key[64] = {0};
...
static void log_rejection(const Candidate* c) {
    ...
    FILE* f = fopen(path, "a");
    if (!f) return;
    /* Rejection log can contain trading intent (symbols, sizing reasons);
     * restrict to owner-only, matching broker.c's order/account files
     * (finding CR6). Best-effort: an open() with O_CREAT would let us set
     * the mode atomically, but fopen()+chmod matches the pattern already
     * used elsewhere in this pipeline stage set. */
    chmod(path, 0600);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int signal_present, price_present;
        cands[i].signal = extract_num_present(objs[i], "signal", &signal_present);
        cands[i].size   = (int)extract_num(objs[i], "size");
        cands[i].price  = extract_num_present(objs[i], "price", &price_present);

        if (!signal_present || !price_present) {
            cands[i].passed = 0;
            snprintf(cands[i].reason, REASON_LEN,
                     "missing required field (%s%s%s)",
                     !signal_present ? "signal" : "",
                     (!signal_present && !price_present) ? "," : "",
                     !price_present ? "price" : "");
        } else {
    ...
    for (int i = 0; i < n; i++) {
        ...
        {
            static const char *reserved[] = {"stage", "language", "risk_status"};
            print_passthrough_meta_fields(objs[i], reserved, 3);
        }
        printf("}\n");
        printf("  }");
    }
