...
int main(int argc, char** argv) {
    ...
    if (wl) {
        ...
        while (tok && n < MAX_SYMBOLS) {
            ...
            /* Match the Python universe.py convention: upper-case symbols
             * so downstream stages don't see case mismatches between the
             * C and Python pipeline implementations (finding CU4). */
            for (char* c = symbols[n]; *c; c++) *c = (char)toupper((unsigned char)*c);
            n++;
            tok = strtok(NULL, ",");
        }
    } else {
