 ...
 *
 * Safety: LIVE mode is refused outright (see FIX PE6 below) -- this
 * stage has no real broker integration. Use the 'order' shell built-in
 ...
 * exit 1 : any order failed, unsized candidate detected, or --mode live
 *
 * Participates in Las_shell pipelines with zero changes to pipes.c.
 * Fully interchangeable with execute.py.
 *
 * For LIVE order routing, use the 'order' shell built-in (src/broker.c),
 * which has real broker integration (see command_order). This stage
 * deliberately does not attempt one -- see the FIX PE6 safety gate below.
 */
...
int main(int argc, char **argv) {
     ...
     * FIX PE6 (promoted from pipeline/execute.py / pipeline/python/execute.py):
     * execute_order_paper() below is a local simulator -- it computes a
     * fabricated fill price and marks every order FILLED regardless of
     * mode, with no actual broker API call anywhere in this file. Before
     * this fix, setting LAS_SHELL_LIVE_CONFIRMED=1 got past the old gate
     * and then silently faked a live fill anyway, no differently from
     * paper mode, with no signal that no real order was placed. Refuse
     * honestly instead of pretending. Real live trading in this project
     * goes through the 'order' shell built-in (src/broker.c), which does
     * have real broker integration (see command_order). */
    if (live_mode) {
        fprintf(stderr,
            "[execute/c] Live execution not implemented in pipeline "
            "execute.c. Use 'order' built-in for live trading.\n");
        return 1;
    }
    ...
    for (int i = 0; i < n_cands; i++) {
        ...
        if (dry_run) {
            ...
            /* Paper mode: local simulation only. Live mode never reaches
             * here -- it's refused earlier in main() (FIX PE6). */
