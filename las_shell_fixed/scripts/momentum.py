#!/usr/bin/env python3
"""
Las_shell Momentum Engine — momentum.py
...
⚠ SYNTHETIC DATA — TESTING ONLY: returns are drawn from an IID Gaussian
per lookback day (seeded by ticker+date), not real price history. This
has no actual autocorrelation or momentum structure, so a "momentum"
signal computed from it doesn't reflect real momentum. Useful for
...
def main():
    ...
    if topn:
        # FIX MO5: topn used to be applied to the full sorted list, so
        # HOLD signals (moderate returns that didn't cross the +-2%
        # threshold) could occupy slots that should go to actual
        # BUY/SELL signals. Filter to actionable signals first, then
        # take the top N of those. (Only applies when --topn is given --
        # without it, the full list including HOLDs is printed as before.)
        actionable = [s for s in scored if s[1] in ("BUY", "SELL")]
        scored = actionable[:topn]
