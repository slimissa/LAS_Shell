...
⚠ SYNTHETIC DATA — TESTING ONLY: prices are a static per-symbol anchor
(SEED_PRICES) perturbed by a single day's random return, reseeded by
ticker+date. Each "day" is an independent perturbation off the same
fixed anchor rather than a compounding price series, so this does not
reflect real historical drift. Useful for exercising the pipeline, not
