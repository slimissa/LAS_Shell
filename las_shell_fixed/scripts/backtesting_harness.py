...
⚠ SYNTHETIC DATA — TESTING ONLY: price/return generation in this file
is simulated (seeded pseudo-randomness), not real historical market
data. Results are useful for exercising the pipeline mechanics, not
for evaluating actual strategy performance. Do not use for production
trading decisions without replacing this with a real data source.
...
def run_backtest(candidates: list) -> dict:
    ...
    if len(pos_rets) > 1:
        ...
        # FIX BT4: sample variance (N-1, Bessel's correction), not
        # population variance (N) -- dividing by N understates true
        # variance for small trade counts, which inflates the reported
        # Sharpe ratio. Needs at least 2 points for N-1 to be valid.
        n = len(pos_rets)
        var_r  = sum((r - mean_r) ** 2 for r in pos_rets) / (n - 1) if n > 1 else 0.0
...
def save_results(result: dict) -> None:
    ...
    # FIX BT6: archive directory for timestamped copies, alongside the
    # canonical (overwritten-each-run) filenames existing consumers rely on.
    archive_dir = os.path.join(LOGDIR, "backtest_archive")
    ...
    if receipts:
        ...
        with open(receipts_path, "w") as f:
            f.write(csv_text)
        # FIX BT6: timestamped archive copy, never overwritten.
        archive_path = os.path.join(archive_dir, f"receipts_{run_ts}.csv")
        with open(archive_path, "w") as f:
            f.write(csv_text)
        print(f"[backtesting_harness] receipts → {receipts_path} (archived: {archive_path})", file=sys.stderr)

    # Summary CSV
    summary_path = os.path.join(LOGDIR, "backtest_harness_summary.csv")
    summary_text = "".join(f"{k},{v}\n" for k, v in summary.items())
    with open(summary_path, "w") as f:
        f.write(summary_text)
    archive_summary_path = os.path.join(archive_dir, f"summary_{run_ts}.csv")
    with open(archive_summary_path, "w") as f:
        f.write(summary_text)
    print(f"[backtesting_harness] summary  → {summary_path} (archived: {archive_summary_path})", file=sys.stderr)
