...
def _state_path(name):
    """FIX MM1: private, non-world-writable directory instead of /tmp,
    with O_NOFOLLOW on open. Same reasoning as GP2 in gen_pairs_orders.py --
    this is persistent per-ticker state re-read on later runs, so the path
    itself can't be randomized away like the streaming FIFO fix (F7)."""
    os.makedirs(STATE_DIR, mode=0o700, exist_ok=True)
    return os.path.join(STATE_DIR, name)

def load_inventory(ticker):
    inv_file = _state_path(f"inventory_{ticker}.txt")
    try:
        fd = os.open(inv_file, os.O_RDONLY | os.O_NOFOLLOW)
...
def save_inventory(ticker, inventory):
    inv_file = _state_path(f"inventory_{ticker}.txt")
    try:
        fd = os.open(inv_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC | os.O_NOFOLLOW, 0o600)
    except OSError as e:
        print(f"market_maker: refusing to write {inv_file}: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    print(f"TICKER={ticker} BID={bid} ASK={ask} MID={mid} SPREAD_BPS={actual_bps} "
          ...
          file=sys.stderr)
    # FIX MM2: downstream stages (risk_check.py, execute) expect the
    # positional "TICKER ACTION SIZE PRICE" format used everywhere else
    # in the pipeline, not the KEY=VALUE diagnostic line above -- emit
    # that on stdout so 'market_maker | risk_filter ?> risk_check.py'
    # actually has something parseable to gate.
    fill_action = "BUY" if fill_side == "BID" else "SELL"
    print(f"{ticker} {fill_action} {size} {fill_price:.2f}")
    sys.exit(0)
