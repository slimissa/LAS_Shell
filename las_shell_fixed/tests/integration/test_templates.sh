#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# Las_shell Strategy Template Library — Test Suite v2
...
SCRIPTS="$LAS_SHELL_HOME/scripts"
TEMPLATES="$LAS_SHELL_HOME/templates"
LOGS="$LAS_SHELL_HOME/logs"
rm -f /tmp/las_shell_inventory_*.txt
rm -f /tmp/las_shell_pairs_*.txt
# FIX (test harness, follows GP2/MM1): market_maker.py and gen_pairs_orders.py
# state moved from predictable /tmp paths to a private ~/.las_shell/state/
# directory (symlink-attack fix). Clean that too so this suite starts from
# a known-empty inventory/direction state, matching what it always assumed
# of the old /tmp path.
rm -rf "${HOME:-/root}/.las_shell/state"
mkdir -p "$LOGS"
...
run_qs() {
    ...
    printf '%s\n' "$script" > "$tmp"
    local out; out=$(LAS_SHELL_HOME="$LAS_SHELL_HOME" "$BINARY" "$tmp" 2>&1); local got=$?
    rm -f "$tmp"
    TOTAL=$((TOTAL+1))
    local pass=1
    [ "$got" = "$want" ] || pass=0
...
echo ""; echo -e "${BOLD}Las_shell Template Library Test Suite v2${RESET}"
echo "Binary     : $BINARY"; echo "LAS_SHELL_HOME: $LAS_SHELL_HOME"; echo "Date       : $(date)"; echo ""
...
# ════════════════════════════════════════════════════════════════
part "PART 2 — Python Scripts (29 tests)"
# ════════════════════════════════════════════════════════════════
...
# FIX (test correctness, follows MO5): --topn filters out HOLD before
# slicing, so the exact line count depends on how many of these 5
# tickers are actionable that day (date-seeded, 0-3) -- not always 3.
# Check the two invariants that ARE always true instead of a fixed count.
TOTAL=$((TOTAL+1))
TOPN_OUT=$(python3 $SCRIPTS/momentum.py AAPL MSFT GOOGL AMZN TSLA --topn 3 2>&1)
TOPN_LINES=$(echo "$TOPN_OUT" | grep -c .)
if [ "$TOPN_LINES" -le 3 ] && ! echo "$TOPN_OUT" | grep -q "HOLD"; then
    echo -e "  ${GREEN}✔${RESET} --topn 3 (<=3 lines, no HOLD)"; PASS=$((PASS+1))
else
    echo -e "  ${RED}✘${RESET} --topn 3 (<=3 lines, no HOLD)"
    echo -e "    output: $TOPN_OUT"; FAIL=$((FAIL+1))
fi
...
o=subprocess.run(['python3','$SCRIPTS/market_maker.py','AAPL'],
...
run_py "pnl text format"  "LAS_SHELL_HOME=$LAS_SHELL_HOME python3 $SCRIPTS/pnl_report.py"       "P&L Report"
run_py "pnl json format"  "LAS_SHELL_HOME=$LAS_SHELL_HOME python3 $SCRIPTS/pnl_report.py --format json | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d[\"total_pnl\"])'" ""
run_py "pnl writes file"  "LAS_SHELL_HOME=$LAS_SHELL_HOME python3 $SCRIPTS/pnl_report.py>/dev/null && cat ~/.las_shell_pnl" ""
run_py "quote streaming" "python3 $SCRIPTS/quote.py AAPL --stream 3 | wc -l | tr -d ' '" "3"

# ════════════════════════════════════════════════════════════════
part "PART 3 — Las_shell Operator Integration (12 tests)"
# ════════════════════════════════════════════════════════════════
section "?> risk gate"
run_qs "small order passes ?>" \
    "setenv LAS_SHELL_HOME $LAS_SHELL_HOME
...
section "|> CSV logging"
CSV1=$(mktemp /tmp/las_shell_XXXXXX.csv)
...
section "Strategy assert logic (Las_shell)"
...
TOTAL=$((TOTAL+1))
mkdir -p "$LOGS"
LAS_SHELL_HOME=$LAS_SHELL_HOME LOGDIR=$LOGS PASS_THRESHOLD=2 python3 $SCRIPTS/run_backtest.py >/dev/null 2>&1
NPERIODS=$(python3 -c "
