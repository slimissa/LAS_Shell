#!/usr/bin/env bash
# =============================================================================
#  test_pipeline.sh — Las_shell Pipeline Convention Test Suite
#  48 tests across 5 parts
...
# =============================================================================
echo -e "${BOLD}Las_shell Pipeline Convention — Test Suite (48 tests)${NC}"
...
# Use --threshold 0.0 so recalculated signals don't filter out test candidates.
# The threshold logic is tested separately by LOW_OUT below.
MOM_OUT=$(echo "$SINGLE" | python3 pipeline/momentum_filter.py --threshold 0.0 2>/dev/null)
assert_valid_json    "momentum_filter output is valid JSON"         "$MOM_OUT"
assert_nonzero       "momentum_filter passes signal=0.8"           "$MOM_OUT"

# Test real threshold: low signal (0.05) with default threshold (0.2) should be empty
...
EXTRA_MOM=$(echo "$EXTRA" | python3 pipeline/momentum_filter.py --threshold 0.0 2>/dev/null)
...
MOM_STAGE=$(echo "$SINGLE" | python3 pipeline/momentum_filter.py --threshold 0.0 2>/dev/null | \
...
assert "signal=0.9 survives risk filter" "$HIGH_COUNT" "1"
...
ALLOC_OUT=$(echo "$MULTI" | python3 pipeline/momentum_filter.py --threshold 0.0 | python3 pipeline/size_positions.py 2>/dev/null)
...
STRAT_OUT=$(echo "$STRAT_IN" | python3 pipeline/momentum_filter.py --threshold 0.0 | \
...
C_BIN=""
for candidate in pipeline/bin/universe pipeline/universe_c pipeline/universe ./universe src/universe; do
...
" 2>/dev/null)
    ...
    MIXED_OUT=$($C_BIN | python3 pipeline/momentum_filter.py --threshold 0.0 | python3 pipeline/size_positions.py 2>/dev/null)
