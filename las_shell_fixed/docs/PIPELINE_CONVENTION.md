...
| `side`   | string  | ✔        | "BUY", "SELL", or "HOLD" (signal == 0.0)                           |
| `meta`   | object  | ✔        | Strategy metadata. Must always contain _convention and stage       |

`meta.timestamp` should be an ISO 8601 string in UTC with at least second
precision (e.g. `2026-03-22T09:30:00Z`) — this is the target format, not
yet what every stage emits (see "Current non-conformance" below).

### `?>` Exit Code Semantics

The `?>` risk gate (implemented in `redirection.c`'s `execute_risk_gate*`
functions) only distinguishes two outcomes at the shell level:

- **0** — the right-hand side (the gate/checker command) exited 0 → PASS.
  The captured left-hand output is forwarded to whatever follows.
- **non-zero** — the gate exited non-zero → REJECT. Nothing runs after the
  gate; the rejection reason is logged to `~/.las_shell_risk_rejections`.

There is currently **no dedicated exit code for "the gate itself failed to
run"** (e.g. the checker script crashed, or a `pipe()`/`fork()` call
failed inside the shell) as distinct from "the gate ran and rejected the
order" — both surface as a non-zero (typically 1) exit and are treated as
REJECT by the shell. If your gate script needs to signal an operational
error differently from a genuine risk rejection, it must currently encode
that in its own stderr output or exit code convention (e.g. `pairs_spread.py`
uses exit 2 internally for its own error case, but that is a convention
local to that one script, not something the shell or `?>` interprets).

### Current Non-Conformance (v0.5.5)

Known places where the shipped code doesn't yet match this convention:

- **Timestamps are not uniformly UTC.** `pipeline/src/universe.c` uses
  `localtime()` and Python's `universe.py` uses `datetime.now()` (also
  local time, plus microsecond precision) — neither emits UTC or a `Z`
  suffix, unlike `audit.c`/`crash_recovery.c` which do.
- **C and Python stage implementations are not numerically interchangeable**
  even though both are synthetic/test data generators seeded the same way
  per symbol: `momentum_filter.c` seeds C's `rand()`, `momentum_filter.py`
  seeds Python's `random` module. Different RNG algorithms, different
  output, same seed. Don't expect a C run and a Python run of "the same"
  stage to produce identical numbers.
...
5. **Exit codes matter** — `?>` risk gate uses exit code to accept/reject (see "`?>` Exit Code Semantics" above for exactly what's distinguished today).
...
| Stage             | Role                                            | Type     | Typical filter         | Implementations                                             |
|-------------------|-------------------------------------------------|----------|------------------------|---------------------------------------------------------------|
| `universe`        | Generates initial candidate list from watchlist | Source   | None (producer)        | `pipeline/universe.py`, `pipeline/src/universe.c`             |
| `momentum_filter` | Keeps signals above threshold, sets signal      | Filter   | \|signal\| >= threshold  | `pipeline/momentum_filter.py`, `pipeline/src/momentum_filter.c` |
| `risk_filter`     | Enforces notional/size/blacklist limits         | Filter   | Risk rules             | `pipeline/risk_filter.py`, `pipeline/src/risk_filter.c`       |
| `size_positions`  | Sets size field based on capital allocation     | Enricher | None (annotator)       | `pipeline/size_positions.py`, `pipeline/src/size_positions.c` |
| `execute`         | Sends orders to broker, emits receipts          | Sink     | None (consumer)        | `pipeline/execute.py`, `pipeline/src/execute.c`               |

**Which copy actually runs:** `run_pipeline.sh` and
`tests/integration/test_pipeline.sh` invoke the **top-level** `pipeline/*.py`
files and the compiled `pipeline/src/*.c` binaries. As of this v0.5.5
docs pass, the top-level `.py` files carry the same fixes that used to be
Python-only in `pipeline/python/` — config-driven risk limits read from
`~/.las_shell_risk`, the `size_positions.py` divide-by-zero guard, and,
importantly, `pipeline/execute.py --mode live` now **refuses honestly**
(exits 1) instead of silently fabricating a fill; there is still no real
broker integration in `execute.py` itself. `pipeline/python/*.py` is now
byte-identical to `pipeline/*.py` and kept only because `make install`
references it — treat `pipeline/*.py` (top level) as the source of truth.
**For live order placement, use the `order` shell built-in**
(`src/broker.c`, requires a `make CURL=1` rebuild plus
`LAS_SHELL_LIVE_CONFIRMED=1`) — `execute.py` cannot place a real order in
any mode. See the main `README.md` "Known Limitations" section.
...
`_convention` is the **pipeline protocol/schema version** ("1.0"), separate
from the LAS_Shell release version (e.g. 0.5.5) — a stage should check
`_convention`, not the shell version, to decide whether it understands a
candidate's shape. Current protocol version: **1.0**. Future protocol
versions remain backward-compatible — new fields are always optional, and
stages must forward unknown fields unchanged. `_convention` only changes
if the schema itself makes a breaking change.
