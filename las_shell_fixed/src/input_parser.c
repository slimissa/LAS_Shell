...
#define TBUF_APPEND(c) do { \
    if (t_len + 1 >= t_cap) { \
        size_t _new_cap = t_cap * 2; \
        char* _tmp = realloc(tbuf, _new_cap); \
        if (!_tmp) { fprintf(stderr, "allocation error\n"); free(tbuf); exit(EXIT_FAILURE); } \
        tbuf = _tmp; t_cap = _new_cap; \
    } \
    tbuf[t_len++] = (c); \
} while(0)
...
#define FLUSH_TOKEN() do { \
    if (t_len > 0) { \
        tbuf[t_len] = '\0'; \
        if (tok_cnt + 1 >= tok_cap) { \
            size_t _new_tcap = tok_cap * 2; \
            char** _tmp2 = realloc(tokens, _new_tcap * sizeof(char*)); \
            if (!_tmp2) { fprintf(stderr, "allocation error\n"); free(tbuf); exit(EXIT_FAILURE); } \
            tokens = _tmp2; tok_cap = _new_tcap; \
        } \
    ...
    while (*p) {
        ...
        if (*p == '>' || *p == '<') {
            ...
            if (*p == '>' && *(p+1) == '>') {
                if (tok_cnt + 1 >= tok_cap) {
                    size_t _nc = tok_cap * 2;
                    char** _t = realloc(tokens, _nc * sizeof(char*));
                    if (!_t) { fprintf(stderr, "allocation error\n"); free(tbuf); exit(EXIT_FAILURE); }
                    tokens = _t; tok_cap = _nc;
                }
