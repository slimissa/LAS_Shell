...
void init_aliases() {
    ...
    if (home) snprintf(alias_path, sizeof(alias_path), "%s/%s", home, ALIAS_FILE);
    else snprintf(alias_path, sizeof(alias_path), "%s", ALIAS_FILE);
...
void save_aliases() {
    ...
    for (int i = 0; i < alias_count; i++) {
        fprintf(f, "%s='", aliases[i].name);
        for (char* p = aliases[i].value; *p; p++) {
            if (*p == '\'') fputs("'\\''", f);
            else fputc(*p, f);
        }
        fprintf(f, "'\n");
...
int remove_alias(char* name) {
    for (int i = 0; i < alias_count; i++) {
        if (my_strcmp(aliases[i].name, name) == 0) {
            ...
            alias_count--;
            /* Clear the now-unused slot so stale pointers can't be reached
             * if alias_count is later mismanaged (defensive hygiene). */
