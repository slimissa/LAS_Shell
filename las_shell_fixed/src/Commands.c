...
char** command_setenv(char** args, char** env) {
    ...
    /* FIX: sync to the real POSIX environment, not just the shell's
     * internal env[] array. Every external command in this codebase is
     * launched via plain execvp(), which inherits the process's actual
     * environment (man environ(7)) -- NOT this internal array, which is
     * only consulted by the shell's own $VAR expansion and a handful of
     * commands that are explicitly handed env[]. Without this, any
     * script doing 'setenv X Y' followed by an external command reading
     * $X via getenv()/os.environ silently sees nothing -- confirmed with
     * a real subprocess test. setmarket/setbroker/setaccount/setcapital
     * already do this exact sync for the same reason; this closes the
     * same gap for the general-purpose builtin. */
...
char** command_unsetenv(char** args, char** env) {
    ...
    /* FIX: same real-OS-environment sync gap as command_setenv above --
     * without this, a variable removed via 'unsetenv X' remains visible
     * to any externally-executed command via getenv()/os.environ. */
    unsetenv(args[1]);
    ...
    for (size_t k = 0; k < i; k++) {
        ...
        if (k != var_index) {
            ...
            new_env[j++] = env[k]; // FIX CM2: transfer ownership, don't dup+leak
...
int command_source(char** args, char*** env_ptr) {
    ...
    extern int execute_script(char* filename, char*** env_ptr);
    return execute_script(args[1], env_ptr);
}
 ...
 * ────────────────────────────────────────────────────────────── */
#define WATCH_POLL_INTERVAL_MS 100
...
int command_watch(char** args, char** env) {
    ...
    while (!get_watch_stop()) {
        ...
        /* Sleep in WATCH_POLL_INTERVAL_MS increments so Ctrl+C is responsive */
        int polls_per_interval = 1000 / WATCH_POLL_INTERVAL_MS;
        for (int i = 0; i < interval * polls_per_interval && !get_watch_stop(); i++) {
            struct timespec ts = { 0, WATCH_POLL_INTERVAL_MS * 1000000L };
            nanosleep(&ts, NULL);
        }
    }
