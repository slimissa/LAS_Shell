 ...
 *
 * Format notes:
 *   - Sections appear in the fixed order ENV, then ALIAS, then JOB; the
 *     parser relies on this order and on the line's key prefix, not on
 *     blank-line delimiters between sections.
 *   - ENV values have embedded newlines escaped as "\n" on save and
 *     unescaped on restore. ALIAS and JOB values are written as-is with
 *     no escaping; embedded newlines in an alias value will corrupt the
 *     format. Values containing '=' are fine (only the first '=' after
 *     the key is treated as the delimiter).
 *   - An empty value (e.g. "ENV=FOO=") is valid and restores FOO as "".
 *
