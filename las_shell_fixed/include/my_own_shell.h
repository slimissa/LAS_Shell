...
typedef struct CommandNode {
    ...
    /* FIX: '{ cmd1; cmd2; ... }' as a single grouped operand of ||/&&/;.
     * Without this, '{ ... }' text got flattened by the operator splitter
     * (which had no brace awareness) into independent top-level sequence
     * nodes -- so a ';' inside the braces behaved like a ';' at the top
     * of the whole line, and anything after ||/&& ran UNCONDITIONALLY
     * instead of only when the left-hand side actually failed/succeeded.
     * A literal 'exit' inside such a block would kill the whole shell
     * regardless of which branch was supposed to run. */
    int   is_group;             // 1 if this node is a '{ ... }' group
    char* group_body;           // the text between the braces, unparsed
} CommandNode;
...
/* handle_sigterm is async-signal-safe: it only sets a volatile
 * sig_atomic_t flag, read back via get_sigterm_received() from normal
 * (non-handler) code. handle_sigint is NOT async-signal-safe — it calls
 * printf() and readline's rl_on_new_line/rl_replace_line/rl_redisplay to
 * redraw the prompt line after Ctrl+C. This is the standard (if strictly
 * non-conforming) pattern for interactive line editors and is accepted
 * here; it should not be used as a template for new signal handlers. */
...
int command_source(char** args, char*** env_ptr);
...
/* FIX RD2/SB2/SS5: absolute path to the running las_shell binary,
 * resolved via /proc/self/exe. Returns NULL if unresolvable, in which
 * case callers should fall back to the old "./las_shell" behavior. */
...
/* Block the calling (main) thread until wall-clock time HH:MM:SS ("@HH:MM:SS")
 * is reached, polling in ~100ms increments so Ctrl+C (watch_stop) can abort
 * it early. Only absolute times are supported (no relative "@+30s" form).
 * Returns 0 once the time is reached or the wait is aborted, -1 if `token`
 * isn't a valid "@HH:MM:SS" string. This blocks all interactive shell use
 * for the duration of the wait — there is no non-blocking/background mode. */
