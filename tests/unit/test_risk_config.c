...
static void test_order_parser(void) {
    ...
    /* FIX RK1: a size without a price can't have its notional validated,
     * so it must be rejected (fail closed), not silently passed. */
    rc = validate_order_against_risk("QQQ SELL 500", &rr);
    TEST("QQQ 500 no price -> REJECTED (RK1 fix)",
...
static void test_notional_limits(void) {
    ...
    /* FIX RK1: no price means notional can't be validated -> reject,
     * don't silently pass a potentially unbounded order through. */
    rc = validate_order_against_risk("SPY BUY 1000", &rr);
    TEST("no price -> REJECTED, can't validate notional (RK1 fix)",
         rc == RISK_ERR_NOTIONAL);
}
