...
# Isolate: use a temp HOME so tests don't pollute user config
TEST_HOME=$(mktemp -d /tmp/las_shell_test_home_XXXXXX)
...
run_script() {
    ...
    local tmp; tmp=$(mktemp /tmp/las_shell_XXXXXX.sh)
    printf '%s\n' "$script" > "$tmp"
...
# ── header ──────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}Las_shell Master Test Suite v4${RESET}"
echo "Binary : $BINARY"
echo "Date   : $(date)"
echo ""
...
TQ=$(mktemp /tmp/las_shell_sq_XXXXXX.sh)
...
TE=$(mktemp /tmp/las_shell_XXXXXX.sh)
...
BN=$(basename "$(mktemp -u /tmp/las_shell_XXXXXX)")
...
section "|> CSV log"
CSV=$(mktemp /tmp/las_shell_XXXXXX.csv)
...
CV2=$(mktemp /tmp/las_shell_XXXXXX.csv)
...
REJLOG="$HOME/.las_shell_risk_rejections"
...
echo \$MSG"
run_script "\$VAR in file path arg"            "hello"        "setenv F /tmp/las_shell_varpath_$$.txt
echo hello > \$F
cat \$F"
rm -f "/tmp/las_shell_varpath_$$.txt"
run_script "\$HOME in arg"                     "$HOME"        "echo \$HOME"

section "~ expansion in args"
run_cmd  "echo ~ → HOME"                       "$HOME"        "echo ~"
run_script "~/file expands correctly"          "ok"           "echo ok > ~/las_shell_tilde_$$.txt
cat ~/las_shell_tilde_$$.txt"
rm -f "$HOME/las_shell_tilde_$$.txt"
run_script "~ in cat arg works"               ""              "cat ~/.las_shell_market" contains
...
CC=$(mktemp /tmp/las_shell_combo_XXXXXX.csv)
...
# ════════════════════════════════════════════════════════════════
# PHASE 3 — ~> BACKTEST OPERATOR (W6 milestones)
# ════════════════════════════════════════════════════════════════
echo ""
echo "── Phase 3: ~> Backtest Operator ───────────────────────────"

# W6 milestone 1: BACKTEST_MODE=1 is injected into env
run_script "~> full date: BACKTEST_MODE=1 set" \
    "BACKTEST_MODE=1" \
    "env | grep BACKTEST_MODE ~> 2020-01-01:2023-12-31"

# W6 milestone 2: BACKTEST_START is injected correctly
run_script "~> full date: BACKTEST_START set" \
    "BACKTEST_START=2020-01-01" \
    "echo test ~> 2020-01-01:2023-12-31
env | grep BACKTEST_START"

# W6 milestone 3: BACKTEST_END is injected correctly
run_script "~> full date: BACKTEST_END set" \
    "BACKTEST_END=2023-12-31" \
    "echo test ~> 2020-01-01:2023-12-31
env | grep BACKTEST_END"

# W6 milestone 4: year shorthand YYYY:YYYY expands correctly
run_script "~> year shorthand: START=2019-01-01" \
    "BACKTEST_START=2019-01-01" \
    "echo test ~> 2019:2024
env | grep BACKTEST_START"

# W6 milestone 5: year shorthand END expands to Dec 31
run_script "~> year shorthand: END=2024-12-31" \
    "BACKTEST_END=2024-12-31" \
    "echo test ~> 2019:2024
env | grep BACKTEST_END"

# W6 milestone 6: ~> prints [~>] backtest mode confirmation
run_script "~> prints backtest mode header" \
    "[~>] backtest mode:" \
    "echo test ~> 2020-01-01:2023-12-31"

# W6 milestone 7: BACKTEST_MODE resets to 0 after ~> completes
run_script "~> resets BACKTEST_MODE=0 after run" \
    "BACKTEST_MODE=0" \
    "echo test ~> 2020-01-01:2023-12-31
env | grep BACKTEST_MODE"

# W6 milestone 8: invalid date range gives error, non-zero exit
run_exit "~> invalid date range exits non-zero" \
    1 \
    "echo test ~> notadate"

# W6 milestone 9: ~> chains with && (success path)
run_script "~> chained with &&" \
    "CHAIN_OK" \
    "echo test ~> 2020:2023 && echo CHAIN_OK"

# W6 milestone 10: python pipeline routes through harness via BACKTEST_MODE
BT_OUT=$(BACKTEST_MODE=1 BACKTEST_START=2020-01-01 BACKTEST_END=2023-12-31 \
         CAPITAL=100000 LAS_SHELL_HOME="$LAS_SHELL_HOME" \
         python3 "$LAS_SHELL_HOME/pipeline/execute.py" \
         <<< '[{"symbol":"SPY","signal":0.5,"size":10,"price":510.0,"side":"BUY","meta":{"_convention":"1.0","stage":"size_positions"}}]' \
         2>&1)
run_test "~> pipeline: execute.py routes to harness" \
    "BACKTEST_MODE=1" \
    "$(echo "$BT_OUT" | grep "routing")"

run_test "~> pipeline: harness emits SIMULATED receipts" \
    "SIMULATED" \
    "$BT_OUT"

run_test "~> pipeline: harness emits backtest summary" \
    "Las_shell Backtest Results" \
    "$BT_OUT"
