...
static char g_daily_baseline_file[512];
...
/* FIX R5: DAILY_LOSS was only readable by assert_risk_limits() if some
 * caller manually setenv'd it first -- nothing in strategies/templates
 * ever did, so the check was practically dead despite the underlying
 * mechanism working correctly. Track a real, persistent, date-aware
 * baseline (today's opening cash) and setenv DAILY_LOSS automatically
 * after every fill, so a bare 'assert' has real data without any
 * strategy script needing to compute and set it manually. */
static void update_daily_loss_tracking(void) {
    home_path(g_daily_baseline_file, sizeof(g_daily_baseline_file),
              ".las_shell_daily_baseline");
    ...
    char stored_date[16] = "";
    double baseline_cash = g_paper.cash;
    int have_baseline = 0;
    int orders_today = 0;

    FILE *f2 = fopen(g_daily_baseline_file, "r");
    if (f2) {
        char line[128];
        while (fgets(line, sizeof(line), f2)) {
            if (!strncmp(line, "DATE=", 5))
                sscanf(line + 5, "%15s", stored_date);
            else if (!strncmp(line, "BASELINE_CASH=", 14)) {
                baseline_cash = atof(line + 14);
                have_baseline = 1;
            } else if (!strncmp(line, "ORDERS_TODAY=", 13)) {
                orders_today = atoi(line + 13);
            }
        }
        fclose(f2);
    }

    if (!have_baseline || strcmp(stored_date, today) != 0) {
        /* new day (or no baseline yet): today's opening cash is the
         * baseline everything else is measured against, and the order
         * counter (FIX RK7) resets to 0 */
        baseline_cash = g_paper.cash;
        orders_today = 0;
    }

    /* FIX RK7: this function is only called after a fill actually
     * succeeds (see paper_execute_buy/sell), so counting one here
     * correctly counts real orders, not attempted/rejected ones. */
    orders_today++;

    f2 = fopen(g_daily_baseline_file, "w");
    if (f2) {
        fchmod(fileno(f2), S_IRUSR | S_IWUSR);
        fprintf(f2, "DATE=%s\nBASELINE_CASH=%.4f\nORDERS_TODAY=%d\n",
                today, baseline_cash, orders_today);
        fclose(f2);
    }

    double loss = baseline_cash - g_paper.cash;  /* positive = lost money today */
    if (loss < 0) loss = 0;
    char loss_str[32];
    snprintf(loss_str, sizeof(loss_str), "%.2f", loss);
    setenv("DAILY_LOSS", loss_str, 1);
    char count_str[16];
    snprintf(count_str, sizeof(count_str), "%d", orders_today);
    setenv("ORDERS_TODAY", count_str, 1);
}

/* FIX RK7: peek-only version for the PRE-trade check in command_order --
 * returns today's order count so far WITHOUT incrementing it (that only
 * happens once a fill actually succeeds, via update_daily_loss_tracking
 * above). Used to reject order #(max+1) before it executes, not just
 * report the count after the fact. */
static int peek_orders_today(void) {
    home_path(g_daily_baseline_file, sizeof(g_daily_baseline_file),
              ".las_shell_daily_baseline");
    ...
    FILE *f = fopen(g_daily_baseline_file, "r");
    if (!f) return 0;
    char line[128];
    ...
    fclose(f);
    return (strcmp(stored_date, today) == 0) ? orders_today : 0;
}
...
static void paper_save(void) {
    paper_ensure_path();
    /* FIX BR3: open() with an explicit mode at O_CREAT time instead of
     * fopen()+fchmod() afterward -- eliminates the narrow window where
     * the file briefly exists at default (world-readable) umask
     * permissions before being restricted. Only matters on first
     * creation; O_TRUNC preserves existing permissions on subsequent
     * saves, same as fopen("w") already did. */
    int fd = open(g_paper_file, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (fd == -1) { perror("broker: cannot save paper account"); return; }
    FILE *f = fdopen(fd, "w");
    if (!f) { perror("broker: cannot save paper account"); close(fd); return; }
...
{
    BrokerResult r = {0};
    /* FIX T2-6.1: defense-in-depth. command_order() already refuses any
     * non-paper order without LAS_SHELL_LIVE_CONFIRMED=1 before dispatch,
     * but this adapter can also be reached directly by future callers
     * (tests, new commands, refactors) that don't go through that gate.
     * Re-check here so a live order can never be placed by this function
     * alone, regardless of how it was invoked. */
    if (!my_getenv("LAS_SHELL_LIVE_CONFIRMED", env)) {
        snprintf(r.message, sizeof(r.message),
                 "refused: LAS_SHELL_LIVE_CONFIRMED not set (alpaca_order)");
        order_log_append(action,ticker,qty,limit,"REJECTED_LIVE_NOT_CONFIRMED",0,"alpaca");
        return r;
    }
...
{
    BrokerResult r = {0};
    /* FIX T2-6.1: defense-in-depth, see alpaca_order() above. */
    if (!my_getenv("LAS_SHELL_LIVE_CONFIRMED", env)) {
        snprintf(r.message, sizeof(r.message),
                 "refused: LAS_SHELL_LIVE_CONFIRMED not set (ibkr_order)");
        order_log_append(action,ticker,qty,limit,"REJECTED_LIVE_NOT_CONFIRMED",0,"ibkr");
        return r;
    }
    const char *base = my_getenv("BROKER_API",       env);
    ...
    /* FIX BR6: only the local Client Portal Gateway (127.0.0.1/localhost)
     * uses a self-signed cert. Disabling verification unconditionally for
     * ANY BROKER_API host means a remote/misconfigured endpoint gets no
     * certificate validation at all -- a live order could be silently
     * intercepted or altered in transit. Check the actual host instead of
     * assuming. */
    {
        int is_local_cpg =
            (strstr(base, "://127.0.0.1") != NULL) ||
            (strstr(base, "://localhost") != NULL);
        curl_easy_setopt(c, CURLOPT_SSL_VERIFYPEER, is_local_cpg ? 0L : 1L);
        curl_easy_setopt(c, CURLOPT_SSL_VERIFYHOST, is_local_cpg ? 0L : 2L);
    }
    CURLcode res = curl_easy_perform(c);
    curl_easy_getinfo(c, CURLINFO_RESPONSE_CODE, &code);
    curl_slist_free_all(hdrs); curl_easy_cleanup(c);
                                   ...
                                   char **env)
{
    ...
    if (!my_getenv("LAS_SHELL_LIVE_CONFIRMED", env)) {
        snprintf(r.message, sizeof(r.message),
                 "refused: LAS_SHELL_LIVE_CONFIRMED not set (generic_order)");
        order_log_append(action,ticker,qty,limit,"REJECTED_LIVE_NOT_CONFIRMED",0,"generic");
        return r;
    }
    const char *base = my_getenv("BROKER_API", env);
    if (!base||!base[0]) {
...
int command_order(char **args, char **env) {
    ...
    const RiskConfig *rc = get_risk_config();
    ...
    /* FIX RK7: MAX_ORDERS_PER_DAY was parsed and shown in `riskconfig show`
     * but never actually enforced anywhere. Check today's real order
     * count (peek, don't increment -- that happens post-fill). */
    if (rc && rc->max_orders_per_day > 0 &&
        peek_orders_today() >= rc->max_orders_per_day) {
        fprintf(stderr,
            "[risk] REJECTED — MAX_ORDERS_PER_DAY %d already reached\n",
            rc->max_orders_per_day);
        order_log_append(action,ticker,qty,limit,"REJECTED_RISK_ORDER_COUNT",0,bn);
        return 1;
    }
    ...
    if (!strcmp(bn,"paper")) {
        ...
        /* FIX BR5: any non-paper path is a real broker call. Require the
         * same explicit confirmation the pipeline's execute.c already
         * requires, so a bare ACCOUNT typo (e.g. "PAPRE") can't silently
         * route a live order — it must be an intentional, confirmed choice. */
        if (!my_getenv("LAS_SHELL_LIVE_CONFIRMED", env)) {
            fprintf(stderr,
                "[broker] REFUSED — ACCOUNT is not exactly 'PAPER' (got broker='%s').\n"
                "  This would place a LIVE order. To confirm, run:\n"
                "    setenv LAS_SHELL_LIVE_CONFIRMED 1\n"
                "  or setaccount PAPER to trade in simulation.\n", bn);
            order_log_append(action,ticker,qty,limit,"REJECTED_LIVE_NOT_CONFIRMED",0,bn);
            return 1;
        }
...
int command_close_all(char **args, char **env) {
    ...
    if (strcmp(bn,"paper")) {
        /* FIX BR10: this used to fetch and print live positions via
         * generic_get_print() and call that "closing" them ("user pipes
         * result through close logic" -- i.e. it never actually
         * submitted any closing order itself). A user running close_all
         * as an emergency live flatten got a printout, not a flatten,
         * with no indication that nothing was actually closed. Refuse
         * honestly instead of pretending -- same fix as PE6. */
        fputs("[broker] close_all: live flattening not implemented. "
              "Use 'order sell TICKER SIZE market' per position for live trading.\n",
              stderr);
        return 1;
    }
...
int command_reset_paper(char **args, char **env) {
    ...
    for (int i=1;args[i];i++) {
        ...
        } else if (!strcmp(args[i],"--yes") || !strcmp(args[i],"--force")) {
            confirmed = 1;
        }
    }
    /* FIX BR4: this permanently deletes all paper positions and P&L
     * history with no undo. Require explicit confirmation. */
    if (!confirmed) {
        if (isatty(fileno(stdin))) {
            printf("This will permanently delete your paper account "
                   "(all positions and P&L history). Type 'yes' to confirm: ");
            fflush(stdout);
            char line[16] = {0};
            if (!fgets(line, sizeof(line), stdin) ||
                strcmp(line, "yes\n") != 0) {
                fputs("reset_paper: aborted\n", stderr);
                return 1;
            }
        } else {
            fputs("reset_paper: refusing without confirmation in a "
                  "non-interactive context. Re-run with --yes to confirm.\n",
                  stderr);
            return 1;
        }
    }
    ...
    paper_save();
    home_path(g_daily_baseline_file, sizeof(g_daily_baseline_file), ".las_shell_daily_baseline");
    unlink(g_daily_baseline_file);  /* fresh baseline (incl. ORDERS_TODAY=0) on next real fill */
    setenv("DAILY_LOSS", "0.00", 1);
    setenv("ORDERS_TODAY", "0", 1);
