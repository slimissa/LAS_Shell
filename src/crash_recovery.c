 ...
 *
 * FIX BUG 3 / FIX CR2: 'checkpoint restore' needs to update the SHELL's
 * live env pointer, not just a local copy. The comment here previously
 * claimed the signature was "extended with env_ptr (char***)" but the
 * actual parameter was still a plain char** -- a stale comment describing
 * a fix that was never actually applied to the code. checkpoint_restore()
 * frees the old env array and writes the replacement only through
 * *env_ptr; with a plain char** parameter that pointer only reached this
 * function's local copy, so the caller (shell_builtins(), and ultimately
 * shell_loop()'s own env) kept referencing freed memory. Confirmed with a
 * real repro: running 'checkpoint restore' interactively (with a
 * checkpoint file containing at least one ENV= line) segfaulted on the
 * very next line that touched env -- same crash shape as MN2. The
 * signature is now genuinely char*** so the caller can pass &env (or,
 * in shell_builtins()'s case, its own already-received env_ptr).
 * ══════════════════════════════════════════════════════════════════════════ */
int command_checkpoint(char **args, char ***env_ptr)
{
    char **env = *env_ptr;
    ...
    if (strcmp(args[1], "restore") == 0) {
        ...
        /* Pass env_ptr straight through -- checkpoint_restore() writes the
         * new array pointer to *env_ptr, which now really does reach the
         * caller's live env slot. */
        CheckpointRestoreResult r = checkpoint_restore(env_ptr,
                                                       cwd_buf,
                                                       sizeof(cwd_buf));
        switch (r) {
