...
int validate_order_against_risk(const char* order_line, RiskResult* result) {
    ...
    /* FIX RK1: previously this check was skipped entirely whenever
     * order.notional was 0.0, which happens whenever price is missing
     * (e.g. a market order line with no price token) -- letting an
     * arbitrarily large order bypass MAX_ORDER_NOTIONAL completely.
     * Fail closed: if there's a real size but no usable price, we can't
     * validate notional at all, so reject rather than silently pass. */
    if (order.size > 0.0 && order.price <= 0.0) {
        result->code = RISK_ERR_NOTIONAL;
        snprintf(result->reason, sizeof(result->reason),
                 "cannot validate notional: order has size %.0f but no price "
                 "(supply a reference price, e.g. a limit price, so "
                 "MAX_ORDER_NOTIONAL can be checked)",
                 order.size);
