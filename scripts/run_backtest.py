...
⚠ SYNTHETIC DATA — TESTING ONLY: market returns used here are a
hardcoded list of quarterly numbers (not a real market data feed), and
the strategy file path only perturbs the random seed rather than being
actually executed against price data. This exercises the pipeline
mechanics (file I/O, CSV output, reporting), not real strategy
performance. Do not use for production trading decisions.
"""
import os, sys, random, hashlib
from datetime import datetime

lhome  = os.environ.get("LAS_SHELL_HOME", ".")
logdir     = os.environ.get("LOGDIR", os.path.join(lhome, "logs"))
...
threshold  = os.environ.get("PASS_THRESHOLD")  # resolved to a number below, once `periods` is known
...
# FIX RB4: resolve the pass threshold now that we know how many periods
# there are. Default to requiring a real majority (>50%) rather than a
# hardcoded 3 (which was only 37.5% of these 8 periods) -- still fully
# overridable via PASS_THRESHOLD for anyone who wants a different bar.
threshold = int(threshold) if threshold else (len(periods) // 2 + 1)
