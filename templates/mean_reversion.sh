...
python3 $QSHELL_HOME/scripts/pairs_spread.py $PAIR_A $PAIR_B --lookback 60 --entry $ENTRY_Z --exit $EXIT_Z |> $LOGDIR/pairs_signals.csv
python3 $QSHELL_HOME/scripts/pairs_spread.py $PAIR_A $PAIR_B --lookback 60 --entry $ENTRY_Z --exit $EXIT_Z

echo ""
echo "── Building orders ──────────────────────────────────────────"
python3 $QSHELL_HOME/scripts/gen_pairs_orders.py

echo ""
echo "── Risk gating orders (?>) ──────────────────────────────────"
python3 $QSHELL_HOME/scripts/gen_pairs_orders.py
echo "AAPL BUY 500 185.50" ?> python3 $QSHELL_HOME/scripts/risk_check.py --max_notional 200000 --max_size 2000 && echo "Risk gate: PASS" || echo "Risk gate: REJECTED"

echo ""
echo "── Verifying hedge ratio ────────────────────────────────────"
python3 $QSHELL_HOME/scripts/check_hedge_ratio.py
